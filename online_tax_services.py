...
class OnlineTaxServices():
    ...
    def CheckOtp(self, otp) -> int:
        if otp == pyotp.TOTP(self.config['USER_INFO']['TOTP_SECRET']).now():
            ...
            self.config['LOGIN_INFO'].update({"ACCESS": self.RequestClientToken()})
    ...
    def CheckTimeout(self) -> int:
        if time.time() - self.activity_timer > self.config['APP_CONFIG']['TIMEOUT']:
            return -1
        else:
            return 0
    ...
    def RequestClientToken(self) -> str:
        #get access key
        try:
            self.session = OAuth2Session(client=BackendApplicationClient(client_id=self.config['APP_CONFIG']['CLIENT_ID']))
            return self.session.fetch_token(token_url=f"{self.config['APP_CONFIG']['BASE_URL']}oauth/token", client_id=self.config['APP_CONFIG']['CLIENT_ID'], client_secret=self.config['APP_CONFIG']['CLIENT_SECRET'], include_client_id=True)['access_token']
        except Exception:
            return 0
    ...
    def ViewReturn(self, period_key) -> int:
        ...
        self.PopulateHeaders()
        self.headers.update({"Authorization": f"Bearer {self.access_token}"})
        self.response = requests.get(f"{self.config['APP_CONFIG']['BASE_URL']}organisations/vat/{self.config['DATABASE_INFO'][self.client_index]['VAT_NO']}/returns/{self.EncodeString(period_key)}", headers=self.headers)
        try:
    ...
    def ShellLiabilities(self) -> int:
        if self.active_client == '' or self.client_index == None:
            print('No client selected!')
            return -1
        st = str(input(f"Enter start date (YYYY-MM-DD): "))
        en = str(input(f"Enter end date (YYYY-MM-DD): "))
            ...
            try:
                ...
                while True:
                    ...
                    if self.CheckTimeout() < 0:
                        print(f"Session timed out. Login again!")
                        sys.exit(0)
                    if cmd == 'help' or cmd == '':
                        self.ShellHelp()
                    elif cmd == 'check':
